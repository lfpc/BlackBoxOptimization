...
local_file_storage = 'local_results.h5'
optimizer = LCSO(
    true_model=problem,
...
t1 = time.time()
surrogate_model.fit(optimizer.local_file_storage)#(local_info[0].reshape(-1, dim+x_dim), local_info[1].reshape(-1, 1))
...
plt.grid(True)
plt.savefig('figs/surrogate_training_loss.png')
...
import h5py
with h5py.File(optimizer.local_file_storage, 'r') as f:
    orig_count = f['conditions'].shape[0]
...
plt.legend(); plt.grid(True); plt.savefig('figs/surrogate_vs_true_loss.png')
...
plt.gca().set_aspect('equal', adjustable='box'); plt.tight_layout(); plt.savefig('figs/calibration_plot_real.png')
...
plt.legend(); plt.grid(True, linestyle='--', alpha=0.6); plt.xlim(0, 1); plt.ylim(0, 1)
plt.gca().set_aspect('equal', adjustable='box'); plt.tight_layout(); plt.savefig('figs/calibration_plot_sampled.png')
...
plt.savefig('figs/probability_histograms.png')
plt.close()
print("Plots saved.")
