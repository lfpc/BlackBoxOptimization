...
class ShipMuonShield():
    ...
    'Piet_solution_7': [
        ...
        [10, 197.0, 18.6, 31.8, 56.0, 56.0, 6.0, 6.0, 8.20, 4.47, 44.0, 44.0, 30.0, 30.0, -1.7],
        [10, 197.0, 31.8, 45.0, 56.0, 56.0, 6.0, 6.0, 4.47, 2.87, 44.0, 44.0, 0.0, 0.0, -1.7],
        [10, 197.0, 31.8, 45.0, 56.0, 56.0, 6.0, 6.0, 4.4764, 2.87, 44.0, 44.0, 0.0, 0.0, -1.7],],
    ...
    "stellatryon_soft_v1": [
    [0.00, 120.50, 40.00, 40.00, 119.00, 119.00, 2.00, 2.00, 1.00, 1.00, 50.00, 50.00, 0.00, 0.00, 1.90],
    [10.00, 275.23, 68.7, 68.7, 20.00, 20.00, 8.00, 8.00, 1.67, 1.67, 77.74, 77.74, 0.00, 0.00, 1.90],
    [10.00, 296.77, 72.9, 72.90, 20.00, 20.00, 8.00, 8.00, 1.52, 1.52, 80.19, 80.19, 0.00, 0.00, 1.90],
    [10.00, 287.71, 59.6, 59.6, 30.00, 30.00, 8.00, 8.00, 2.22, 2.22, 66.49, 66.49, 0.00, 0.00, 1.90],
    [10.00, 113.85, 20,    20, 30.00, 30.00, 0.00, 0.00, 8, 8, 10.41, 10.41, 0.00, 0.00, -0.00], # changed by hand the height and core
    [10.00, 168.00, 20.00, 30.00, 40.00, 40.00, 28.79, 11.75, 6.16, 4.34, 33.00, 33.00, 0.00, 0.00, -1.2],
    [10.00, 193.16, 51.59, 83.99, 50.00, 50.00, 8.00, 8.00, 2.87, 1.38, 92.39, 92.39, 0.00, 0.00, -1.87],
],

    "stellatryon_soft_v1_cheaper": [
[  0.0000, 120.5000,  40.0000,  40.0000, 119.0000, 119.0000,   2.0000,
          2.0000,   1.0000,   1.0000,  50.0000,  50.0000,   0.0000,   0.0000,
          1.9000],
[ 10.0000, 264.9403,  61.6833,  61.6833,  13.9129,  13.9129,   8.3320,
         10.6594,   1.0377,   1.0000,  75.5700,  75.5700,   0.0000,   0.0000,
          1.9000],
[ 10.0000, 284.9163,  66.1431,  66.1431,  14.5451,  14.5451,   9.0032,
          9.1093,   1.3341,   1.3325,  80.1900,  80.1900,   0.0000,   0.0000,
          1.9000],
[ 10.0000, 300.0000,  44.8037,  44.8037,  10.0000,  10.0000,   8.0000,
          8.6359,   2.2774,   2.2632,  65.5600,  65.5600,   0.0000,   0.0000,
          1.9000],
[ 10.0000, 100.0000,  20.0000,  20.0000,  30.0000,  30.0000,   0.0000,
          0.0000,   5.1375,   5.1375,  22.0000,  22.0000,   0.0000,   0.0000,
         -0.0000],
[ 10.0000, 168.0000,  20.0000,  30.0000,  40.0000,  40.0000,  24.0871,
          8.1476,   4.8382,   3.4235,  33.0000,  33.0000,   0.0000,   0.0000,
         -1.2000],
[ 10.0000, 184.7712,  39.8741,  74.4337,  45.0193,  45.0193,   8.0896,
          8.0000,   3.0319,   1.1611,  81.8771,  81.8771,   0.0000,   0.0000,
         -1.8826],
],
    "stellatryon_TRY4": [
[  0.0000, 120.5000,  40.0000,  40.0000, 119.0000, 119.0000,   2.0000,
...
[ 10.0000, 190.0629,  37.9501,  74.4335,  45.0191,  45.0191,   8.0000,
    ...
    parametrization = {
        ...
        "stellatryon_soft": (
            make_index(1, [1,2,3,4,9]) +
            make_index(2, [1,2,3,4,9]) +
            make_index(3, [1,2,3,4,9]) +
            make_index(4, [1,4,9]) +
        ...
        "stellatryon_rect_soft": (
            make_index(1, [1,2,4,6,7,9]) +
            make_index(2, [1,2,4,6,7,9]) +
            make_index(3, [1,2,4,6,7,9]) +
            make_index(4, [1,9]) +
            make_index(5, [6,7,9]) +
            make_index(6, [1,2,3,4,6,7,9,14])
        ),
        # 24 parameters
        "stellatryon_last_magnet_soft": (
            make_index(6, [1,2,3,6,7,9,14])
        ),
    }
    ...
    def GetBounds(self,device = torch.device('cpu')):
        z_gap = (10,50)
        magnet_lengths = (100, 300)
        dY_bounds = (10, 60)
        dY_yoke_bounds = (10, 450)
        if self.use_B_goal: NI_bounds = (0,1.9)
        else: NI_bounds = (0.0, 70e3)
        if self.use_diluted:
            dX_bounds = (10, 85)
            gap_bounds = (8, 30)
            inner_gap_bounds = (0., 30.)
            yoke_bounds = (1,10) if self.use_ratio_yoke else (dX_bounds[0], dX_bounds[1]*3)
        else:
            dX_bounds = (5, 250)
            gap_bounds = (2, 150)
    ...
    def add_fixed_params(self, phi: torch.Tensor):
        if phi.numel() != (self.n_magnets * self.n_params):
            ...
            # new_phi[:,13] = new_phi[:,12] MIDDLE-GAP IDENTICAL ENTER-EXIT
            new_phi = new_phi.index_put(
                (rows, torch.tensor([13])),
                new_phi[rows, 12]
            )
            ...
            if self.use_diluted:
                ...
                if self.key and self.key.startswith("piet"):
                    ...
                    if self.n_magnets <7:                    
                        ...
                        piet = torch.tensor(self.params['Piet_solution_7'], dtype = new_phi.dtype, device=new_phi.device)
                    ...
                    rows_S1 = torch.arange(1, new_phi.size(0)-3, device=new_phi.device)
                    #∆Xcore,2 = ∆Xcore,1
                    new_phi = new_phi.index_put(
                                                (rows_S1, torch.tensor([3])),
                                                new_phi[rows_S1, 2]
                                            )
                    ...
                    new_phi.index_put_((rows, torch.tensor([8])), values_8)
    ...
    def get_constraints(self,phi):
        ...
        print("total constraint:", constraints)
    ...
    def simulate(self,phi:torch.tensor,muons = None, return_all = False): 
        ...
        try: 
            output = self.run_muonshield(phi.numpy(), 
                                        ...
                                        SmearBeamRadius=self.SmearBeamRadius,
                                        fSC_mag = self.fSC_mag,
                                        simulate_fields = not self.uniform_fields if self.fields_file == None else False,
                                        field_map_file = self.fields_file,
