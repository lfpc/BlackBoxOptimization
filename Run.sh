...
read -p "Select optimization method (GA/RL/toyRL/RLfinal/CMAES/cma-es/CEM/bayesian): " OPT_METHOD
if [[ "$OPT_METHOD" != "GA" && "$OPT_METHOD" != "RL" && "$OPT_METHOD" != "toyRL" && "$OPT_METHOD" != "RLfinal" && "$OPT_METHOD" != "CMAES" && "$OPT_METHOD" != "cma-es" && "$OPT_METHOD" != "CEM" && "$OPT_METHOD" != "bayesian" ]]; then
    echo "Error: Invalid choice. Must be 'GA', 'RL', 'toyRL', 'RLfinal, 'CMAES', 'cma-es', 'CEM' or 'bayesian'."
    exit 1
fi
...
nohup apptainer exec --nv \
  ...
  bash --login -i -c "
    ...
    python3 -V
    CUDA_VISIBLE_DEVICES=$CUDA_DEVICE python3 run_optimization.py --optimizer $OPT_METHOD --name $RESULTS_NAME  --config_file outputs/config_50M.json
  " > "$LOG_FILE" 2>&1 &
