...
read -p "Select optimization method (GA/RL/toyRL/RLfinal/CMAES/CEM/bayesian): " OPT_METHOD
if [[ "$OPT_METHOD" != "GA" && "$OPT_METHOD" != "RL" && "$OPT_METHOD" != "toyRL" && "$OPT_METHOD" != "RLfinal" && "$OPT_METHOD" != "CMAES" && "$OPT_METHOD" != "CEM" && "$OPT_METHOD" != "bayesian" ]]; then
    echo "Error: Invalid choice. Must be 'GA', 'RL', 'toyRL', 'RLfinal, 'CMAES', 'CEM' or 'bayesian'."
    exit 1
fi
