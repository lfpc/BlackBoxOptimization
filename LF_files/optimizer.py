...
class LCSO_sampled(LCSO):
    ...
    def simulate_and_update(self, phi, update_history:bool = True, file = None):
        ...
        phi_denorm = denormalize_vector(phi, self.bounds).detach().cpu()
        y = self.true_model(phi_denorm,x)
        if file is not None:
            ...
            self.local_results[0].append(phi)
            self.local_results[1].append(x)
            self.local_results[2].append(y)
        loss = self.n_hits(y, self.true_model.get_weights(x)).reshape(1)
        if update_history:
            self.update_history(phi, loss.detach())
        return loss
    def _get_local_data(self):
        phis = self.sample_phi()
        phis = torch.cat([self._current_phi.view(1,-1),phis], dim=0)
        if self.local_file_storage is not None:
            file = h5py.File(self.local_file_storage, 'a', libver='latest')
            len_data = file.attrs.get('write_pos', 0)#file['condition'].shape[0] if 'condition' in file else 0
        else: 
    ...
    def optimization_iteration_second_order(self, debug = False):
        ...
        surrogate_grad = grad_fn(current_phi)
        def hvp_func(v, lam = 1e-3):
            _, hvp_val = torch.func.jvp(grad_fn, (current_phi,), (v,))
            return hvp_val + lam * v # LM damping for stability
...
class RL_muons_env_final(gym.Env):
    ...
    def step(self, action):
        ...
        if terminated:
            ...
            print("GPU alloc:", torch.cuda.memory_allocated(self.device))
            print("GPU reserved:", torch.cuda.memory_reserved(self.device))
            reward = -torch.log(1.0+self.problem_fn(phi))/10#Consider reward=-log(1+loss)/10 to deal with huge losses
...
class RL_final():
    ...
    def run_optimization(self):#TO_DO: Should I fix log_std?
        #def make_env(problem_fn, device):
        #    def _init():
        #        torch.cuda.set_device(device)
        #        return RL_muons_env_final(problem_fn, device)
        #    return _init
        def make_env(rank, problem_fn, devices):
            def _init():
                device = devices[rank % len(devices)]#[0]
                torch.cuda.set_device(device)
                env = RL_muons_env_final(problem_fn, device)
                return env
            return _init
        #train_env = DummyVecEnv([make_env(self.problem_fn, self.devices[0])])
        ...
        #for i in range(2):
        #En 11 comprobar que los workers finalizan bien con 24 envs (uniform fields). En 13 comprobar que el SB3 training se completa con 24 envs (no uniform fields)
        #model.learn(total_timesteps=self.training_steps//2,callback=callback,reset_num_timesteps=False)
        #train_env.close()
        #train_env = SubprocVecEnv([make_env(rank=i, problem_fn=self.problem_fn, devices=self.devices) for i in range(self.SB3_num_envs)],start_method="spawn")
        #model.learn(total_timesteps=self.training_steps//2,callback=callback,reset_num_timesteps=False)
        if self.algorithm=="SB3_TQC":
